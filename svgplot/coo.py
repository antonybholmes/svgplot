...
def add_coo_plot(svg,
                    ...
                    showlabels=True,
                    showblocks=True,
                    colormap=libsvg.ABC_COLORS,
                    frame=False,
                    framecolor='black',
                    stroke=libsvg.STROKE_SIZE,
                    sigqcolor='#ffffff',
                    invert_x=False):
    #if smallfont:
    #    svg.set_font_size(libsvg.FIGURE_FONT_SIZE)
    ...
    bar_block = (bar_width * df_coo.shape[1]) + (bar_gap * (df_coo.shape[1] - 1)) + (block_gap_t + block_gap_b) 
    ...
    svg.add_line(x1=0, y1=y, x2=0, y2=y2, stroke=libsvg.AXIS_STROKE)
    
    if showxaxis:
        svg.add_x_axis(axis=xaxis, 
                        y=y2, 
                        ticks=xticks, 
    ...
    #svg.set_font_size(libsvg.DEFAULT_FONT_SIZE)
...
def add_hits_plot(svg,
    ...
    bar_block = (bar_width * classes.size) + (bar_gap * (classes.size - 1)) + (2 * block_gap) 
    ...
    for row_block in row_labels:
        for label in row_block['items']:
            ...
            for j in range(0, idx.size):
                ...
                if sig:
                    #svg.set_font_size(libsvg.HEADING_FONT_SIZE)
                    svg.add_text_bb('*', 
                                        x=x3, 
                                        w=bar_width, 
                                        y=y2 - yaxis.scale(v) - svg.get_font_h() / 2, 
                                        align='c')
    ...
    svg.add_line(x1=0, y1=y2, x2=w, y2=y2, stroke=libsvg.AXIS_STROKE)
    ...
    if showlabels:
        svg.cluster_label_cols(row_labels, clusters, y=y2, w=w, showgroups=False)
...
def add_enrich_dep_plot(svg,
    ...
    bar_block = (bar_width * df_coo.shape[1]) + (bar_gap * (df_coo.shape[1] - 1)) + (block_gap[0] + block_gap[1]) 
    ...
    for row_block in cluster_groups:
        for label in row_block['items']:
            ...
            for j in range(df_coo.shape[1]):
                ...
                svg.add_rect(x2, y1, w=bar_width, h=h, fill=color, stroke=stroke)
            ...
            if bar_border is not None:
                svg.add_rect(x=bx, y=y-height, w=bar_width*df_coo.shape[1], h=height, color=bar_border, stroke=stroke)
            
            bx = x2
                
                
                #break
        svg.add_line(x1=x1, y1=y, x2=x2, y2=y, stroke=libsvg.AXIS_STROKE)
        svg.add_line(x1=x1, y1=y2, x2=x2, y2=y2, dashed=True, stroke=libsvg.AXIS_STROKE)
    ...
    #svg.add_line(x1=x, y1=y, x2=x2, y2=y, stroke=libsvg.AXIS_STROKE)
    ...
    svg.add_text_bb('Enrichment', x=x2-20, y=y-height/2, size=libsvg.FIGURE_FONT_SIZE, orientation='v', align='c')
    ...
    for row_block in cluster_groups:
        ...
        for label in row_block['items']:
            ...
            if bar_border is not None:
                svg.add_rect(x=bx, y=y, w=bar_width*df_coo.shape[1], h=height, color=bar_border)
    ...
    svg.add_text_bb('Depletion', x=x2-20, y=y+height/2, size=libsvg.FIGURE_FONT_SIZE, orientation='v', align='c')
    ...
    for row_block in cluster_groups:
        for label in row_block['items']:
            ...
            for j in range(df_coo.shape[1]):
                ...
                svg.add_rect(x2 + bar_gap, y1, bar_width-2*bar_gap, h, fill=color, stroke=stroke)
                
                if bar_color is not None:
                    svg.add_rect(x2 + bar_gap, y1, bar_width-2*bar_gap, h, color=bar_color, stroke=stroke)
            ...
            if bar_border is not None:
                svg.add_rect(x2 + bar_gap, y1, bar_width-2*bar_gap, height, color=bar_border, stroke=stroke)
            ...
            x2 += bar_width
                ...
                #break
        svg.add_line(x1=x1, y1=y+height, x2=x2, y2=y+height, stroke=libsvg.AXIS_STROKE)
        x2 += cluster_group_gap
        x1 = x2
