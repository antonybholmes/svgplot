    ...
    for f in files:
        ...
        for i in range(0, gene_sets.size):
            for j in range(0, cluster_sets.size):
                ...
                cid = cluster_sets[j]  # clusters.display_id_to_id(cluster_sets[j])

                d[i, j] += (
                    c
                    if np.where(
                        (df["Gene Set"] == gene_sets[i]) & (df["Cluster"] == cid)
                    )[0].size
                    > 0
                    else 0
                )
    ...
    for i in range(0, gene_sets.size):
        ...
        for j in range(0, cluster_sets.size):
            ...
            if v == 1:
                ...
                color = pathwaycolor[1]
                svg.add_rect(hx + cell[0] / 2, hy, cell[0] / 2, cell[1], fill=color)
            else:
                color = "white"
                svg.add_rect(hx, hy, cell[0], cell[1], fill=color)
