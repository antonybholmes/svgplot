...
#        spacings[-1] = 0
    ...
    def add_svg_colorbar(self,
        ...
        self.add_rect(x=x+w/4, y=y, w=w/2, h=h, fill=svgplot.rgbtohex(
        ...
        if showframe:
            self.add_frame(x, y=y, w=w, h=h, stroke=stroke)

    def add_vert_svg_colorbar(self,
                              x=0,
                              y=0,
                              w=25,
                              h=200,
        ...
        for step in steps:
            ...
            h1 = h - yoff
            col = svgplot.rgbtohex(cmap(int(norm(step) * cmap.N - 1)))
            self.add_rect(x=x, y=y, w=w, h=h1, fill=col)
            #self.add_line(x1=x+xoff, y1=y, x2=x+xoff, y2=y+h)
