...
def add_single_split_heatmap(
    ...
    for i in range(0, len(row_sets)):
        ...
        if name is not None:
            h = len(genes) * cell[1]
            svg.add_text_bb(name, x=hx, y=hy + h, w=h, align="c", orientation="v")
...
def add_single_cell_col_labels(
    svg: SVGFigure,
    clusters,
    col_sets,
    pos: tuple = (0, 0),
        ...
        for i in range(0, len(col_set)):
            ...
            for mode in modes:
                if mode == "short":
                    ...
                    group = (
                        clusters.get_group(altId) if clusters is not None else str(col)
                    )
                else:
                    group = altId

                color = clusters.get_color(group) if clusters is not None else "black"
                block_color = (
                    clusters.get_block_color(group) if clusters is not None else "black"
                )
            ...
            for g in group_map["short"]:
                ...
                if grouporientation == "a":
                    ...
                    gx2 = (
                        gx
                        + gw / 2
                        - (svg.get_font_h() + linespacing) / 2 * (len(words) - 1)
                    )
                    # gx2 = gx
                    for word in words:
                        svg.add_text_bb(
