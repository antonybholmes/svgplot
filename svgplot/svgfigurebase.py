...
class SVGFigureBase:
    def __init__(
        ...
        self._svg = svgwrite.Drawing(
            file, size=size, viewBox="0 0 {} {}".format(view[0], view[1])
        )
        ...
        self._grid = grid
        self._int_view = (view[0] - 2 * self._border[0], view[1] - 2 * self._border[1])
        self._grid_xy = (self._int_view[0] / grid[1], self._int_view[1] / grid[0])
        self._subgrid_xy = (self._grid_xy[0] / grid[1], self._grid_xy[1] / grid[0])
        self._offset = [0, 0]
    ...
    def set_sub_cell(self, row: int = 0, col: int = 0):
        self.set_sub_offset(col * self._subgrid_xy[0], row * self._subgrid_xy[1])
    ...
    def t(
        self, x: Optional[float] = None, y: Optional[float] = None, inc: bool = False
    ) -> None:
        if x is not None:
            if inc:
                x += self._trans[-1][0]
    ...
    def scale(
        ...
        g = self._svg.g(
            transform=self.format_scale(x, y), style=core.format_css_params(css)
        )
    ...
    def trans(
        ...
        g = self._svg.g(
            transform=self.format_translate(self.x(x), self.y(y)),
    ...
    def add_rot_trans(
        self, elem, x: float = 0, y: float = 0, rotate: float = 0
    ) -> None:
    ...
    def get_string_width(
        ...
        return (
            core.get_text_metrics(label, family=family, size=size, weight=weight)[0]
            * PX_TO_SVG
        )

    def _get_font_center_x(
        self,
        label,
        x,
        w,
        ...
        sw = self.get_string_width(label, family=family, size=size, weight=weight)
    ...
    def get_font_h(
        ...
        return (
            core.get_text_metrics("A", family=family, size=size, weight=weight)[1] * 2.7
        )
    ...
    def get_font_y(
        ...
        # * (1 - (1 - get_font_h(s)) / 2)
        return (
            y + h - (h - self.get_font_h(family=family, size=size, weight=weight)) / 2
        )
    ...
    def css_map(
        self, css: Optional[dict[str, Union[str, int, float]]]
        ...
        if "font-size" not in css:
            css["font-size"] = "{}px".format(self.get_scale_font_size(self._font_size))
    ...
    def add_title(
        ...
        if subtext is not None:
            ...
            self.add_text_bb(
                subtext, x=x, y=y, w=w, align="c" if twolines else "l", color=color
            )
    ...
    def add_section(
        ...
        self.add_text(
            "{}".format(
                label.upper() if mode == "upper" else label.lower()
            ),  # upper()),
            x=x,
            y=self.get_font_y(y, h, size=self._heading_font_size, weight=weight),
            h=h,
    ...
    def add_fig_name(self, id, align="l", mode="long", weight="bold") -> None:
        ...
        if mode == "short":
            self.add_fig_title("Fig. {}".format(id), align=align, weight=weight)
        else:
            self.add_fig_title("Figure {}.".format(id), align=align, weight=weight)

    def add_fig_title(
        self, title, align: str = "l", size: int = 10, weight: str = "normal"
    ) -> None:
        """
    ...
    def add_text_bb(
        ...
        if frameargs["style"] is not None:
            if frameargs["style"] == "circle":
                x1 = x  # + (w - frameargs['s'])
                self.add_circle(x=x1, y=y, w=frameargs["s"], color=frameargs["color"])
