...
class SVGFigureHeatmap(SVGFigurePlot):
    ...
    def heatmap_color_cols(
        ...
        for i in range(0, df_heatmap.shape[1]):
            ...
            self.add_rect(x + i * xs, gy, xs, core.LABEL_COLOR_BLOCK_SIZE, fill=color)
            ...
            if frame:
                self.add_frame(
                    x + i * xs, gy, xs, core.LABEL_COLOR_BLOCK_SIZE, color="black"
                )
        ...
        if labels:
            ...
            for g in group_map:
                ...
                if (
                    (orientation == "auto") and (len(group_map[g]) < 3 and len(g) > 2)
                ) or orientation == "v":
                    self.add_text_bb(
                        g, gx, gy, w=gw, color=color, orientation="v", weight=weight
                    )
                else:
                    self.add_text_bb(
                        g, gx, gy, w=gw, align="c", color=color, weight=weight
                    )
    ...
    def heatmap_color_cols_v(
        ...
        for i in range(0, df_heatmap.shape[1]):
            ...
            self.add_rect(x, y + i * xs, core.LABEL_COLOR_BLOCK_SIZE, xs, fill=color)
            self.add_frame(
                x, y + i * xs, core.LABEL_COLOR_BLOCK_SIZE, xs, color="black"
            )
        ...
        if labels:
            for g in group_map:
                ...
                self.add_text_bb(
                    g,
                    x=x + 2 * core.LABEL_COLOR_BLOCK_SIZE,
                    y=gx,
                    w=gw,
                    color=color,
                    weight=weight,
    ...
    def heatmap_color_rows(
        self, file, df, clusters, x=0, y=0, w=0, h=None, gap=10, framecolor=None
    ):
        if h is None:
        ...
        for id in ids:
            ...
            self.add_rect(gx, y, core.LABEL_COLOR_BLOCK_SIZE, hc, fill=color)
            ...
            if c > 0 and gap == 0:
                ...
                self.add_line(
                    x1=gx,
                    y1=y2,
                    x2=gx + core.LABEL_COLOR_BLOCK_SIZE,
                    y2=y2,
                    color="white",
                )

            if framecolor is not None:
                self.add_frame(gx, y, core.LABEL_COLOR_BLOCK_SIZE, hc, color=framecolor)

            y += hc + gap
            c += 1

    def heatmap_label_rows(
        self,
        file,
        df,
        genes,
                ...
                if idx.size > 0:
                    if i == 0:
                        ...
                        if show_ticks:
                            ...
                            self.add_line(gx - 20, gy, gx - 10, gy, color="black")
    ...
    def add_heatmap(
        ...
        if showgrid:
            self.add_grid(
                x=x, y=y, w=w, h=h, rows=df.shape[0], cols=df.shape[1], color=gridcolor
            )
    ...
    def add_scaled_heatmap(
        ...
        for id in clusters.get_clusters():
            ...
            if framecolor is not None:
                self.add_frame(
                    x=x, y=starty, w=w, h=dh * dfc.shape[0], color=framecolor
                )
    ...
    def heatmap_color_cols_scaled(
        ...
        if labels:
            ...
            for g in group_map:
                ...
                if (
                    orientation != "a"
                    and (len(group_map[g]) < 3 and len(g) > 2)
                    or orientation == "v"
                ):
                    self.add_text_bb(
                        g, gx, gy, w=gw, color=color, orientation="v", weight=weight
    ...
    def add_split_heatmap(
        ...
        df1.columns = [
            x.replace("Cluster ", "")
            .replace(" log2 fold change", "")
            .replace(" avg log2(tpm + 1)", "")
            ...
            for i in range(0, len(col_sets)):
                ...
                if i < len(headings):
                    ...
                    hx2 = (
                        hx
                        + w / 2
                        - 0.5 * (self.get_font_h() + padding) * (len(words) - 1)
                    )
                ...
                if (
                    i >= len(headings)
                    or "nospace-after" not in headings[i]
                    or not headings[i]["nospace-after"]
                ):
                    hx += hspace
        ...
        for i in range(0, len(row_sets)):
            ...
            for j in range(0, len(col_sets)):
                ...
                for col in col_set:
                    ...
                    for gene in genes:
                        if group == 1 or group == "a":
                            ...
                            ci = np.where(df1.columns == "C{}".format(cluster))[0][0]
                            v = df1.iloc[ri, ci]
                        else:
                            ri = np.where(df2.index == gene)[0][0]
                            ci = np.where(df2.columns == "C{}".format(cluster))[0][0]
                            v = df2.iloc[ri, ci]
                ...
                # if showframe:
                if j == 0 or (
                    "nospace-after" not in headings[j]
                    and "nospace-after" not in headings[j - 1]
                ):
                ...
                if (
                    j >= len(headings)
                    or "nospace-after" not in headings[j]
                    or not headings[j]["nospace-after"]
                ):
                    hx2 += hspace
        ...
        for i in range(0, len(row_sets)):
            ...
            if name is not None:
                h = len(genes) * cell[1]
                self.add_text_bb(name, x=hx, y=hy + h, w=h, align="c", orientation="v")
            ...
            for i in range(0, len(row_sets)):
                ...
                for col in col_set:
                    ...
                    for gene in genes:
                        ...
                        ri = np.where(df1.index == gene)[0][0]
                        ci = np.where(
                            (df1.columns == col) | (df1.columns == "C{}".format(col))
                        )[0][0]
                        v = df1.iloc[ri, ci]
    ...
    def heatmap_label_cols(
        ...
        group_map = collections.defaultdict(lambda: collections.defaultdict(set))
        color_map = {}
        block_color_map = {}
        ...
        for col_set in col_sets:
            ...
            for i in range(0, len(col_set)):
                ...
                altId = clusters.find(col) if clusters is not None else str(col)

                color = clusters.get_color(altId) if clusters is not None else "black"

                for mode in modes:
                    if mode == "short":
                        group = (
                            clusters.get_short_group(altId)
                            if clusters is not None
                            else str(col)
                        )
                    elif mode == "group":
                        group = (
                            clusters.get_group(altId)
                            if clusters is not None
                            else str(col)
                        )
                    else:
                    ...
                    color = (
                        clusters.get_color(group) if clusters is not None else "black"
                    )
                    block_color = (
                        clusters.get_block_color(group)
                        if clusters is not None
                        else "black"
                    )
            ...
            if "bars" in group_map:
                ...
                for g in group_map["bars"]:
                    if g not in ignore:
                        ...
                        if c > 0:
                            self.add_line(
                                x1=gx, y1=gy - 2, x2=gx, y2=gy + h + 2, color="white"
                            )
            ...
            if "short" in group_map:
                ...
                for g in group_map["short"]:
                    ...
                    if grouporientation == "a":
                        ...
                        (grouporientation == "auto")
                        and (len(group_map["short"][g]) < 3 and len(g) > 1)
                    ) or grouporientation == "v":
                        ...
                        gx2 = gx - (self.get_font_h() + linespacing) / 2 * (
                            len(words) - 1
                        )
            ...
            if "bars" in group_map:
                ...
                for g in group_map["bars"]:
                    if g not in ignore:
                        ...
                        if c > 0:
                            self.add_line(x1=gx, y1=gy, x2=gx + w, y2=gy, color="white")
            ...
            if "clusters" in group_map:
                ...
                for g in group_map["clusters"]:
                    if g not in ignore:
                        ...
                        if orientation == "v":
                            self.add_text_bb(
                                g, x=gx, y=gy, h=cell[1], color=color, weight=weight
                            )

                            sw = max(sw, self.get_string_width(g))
                        else:
                            self.add_text_bb(
                                g, gx, gy, h=cell[1], color=color, weight=weight
                            )
            ...
            if "short" in group_map:
                ...
                for g in group_map["short"]:
                    ...
                    if grouporientation == "a":
                        ...
                        self.add_text_bb(
                            g, x=gx, y=gy2, h=gh, color=color, weight=weight
                        )
                    elif (
                        (grouporientation == "auto")
                        and (len(group_map["short"][g]) < 3 and len(g) > 1)
                        ...
                        gy2 = gy - (self.get_font_h() + linespacing) / 2 * (
                            len(words) - 1
                        )

                        for word in words:
                            self.add_text_bb(
                                word, x=gx, y=gy2, w=gh, color=color, weight=weight
                            )
                        ...
                        if showunclass or "unclass" not in g.lower():
                            self.add_text_bb(
                                g, gx, gy, w=gh, color=color, weight=weight
                            )
    ...
    def pathway_heatmap(
            ...
            for i in range(0, gene_sets.size):
                for j in range(0, cluster_sets.size):
                    ...
                    d[i, j] += (
                        c
                        if np.where(
                            (df["Gene Set"] == gene_sets[i])
                            & (df["Cluster"] == "C{}".format(cid))
                        )[0].size
                        > 0
                        else 0
                    )
        ...
        for i in range(0, gene_sets.size):
            ...
            for j in range(0, cluster_sets.size):
                ...
                if v == 1:
                    ...
                    color = pathwaycolor[1]
                    self.add_rect(
                        hx + cell[0] / 2, hy, cell[0] / 2, cell[1], fill=color
                    )
                else:
                    color = "white"
                    self.add_rect(hx, hy, cell[0], cell[1], fill=color)
        ...
        if showgrid:
            self.add_grid(
                x=x, y=y, w=hx, h=hy, rows=d.shape[0], cols=d.shape[1], color=gridcolor
            )
        ...
        for group in row_labels:
            ...
            for item in group["items"]:
                ...
                if showlabels:
                    ...
                    if align == "left":
                        self.add_text_bb(item, x=x - tw - 10, y=y, h=yd, color=color)
                    else:
                        self.add_text_bb(item, x=x + w + 10, y=y, h=yd, color=color)
    ...
    def add_grid(
        ...
        if drawrows:
            ...
            for i in range(1, rows):
                self.add_line(x1=x, y1=y, x2=x + w, y2=y, color=color, stroke=stroke)
        ...
        if drawcols:
            ...
            for i in range(1, cols):
                self.add_line(x1=x, y1=y, x2=x, y2=y + h, color=color, stroke=stroke)
