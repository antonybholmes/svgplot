...
def kws(
    kws: dict[str, Any], user_kws: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
...
DHIT_COLORS = {
    "DHITsig-pos": rgbatohex(matplotlib.cm.get_cmap("tab20c")(16)),
    "DHITsig-neg": rgbatohex(matplotlib.cm.get_cmap("tab20c")(18)),
    "n/a": "none",
}  # 'lightgray'{'DHITsig-pos':'dimgray', 'DHITsig-neg':'silver'}
...
def css_params(*params):
    return "; ".join(
        [css_param(params[i], params[i + 1]) for i in range(0, len(params), 2)]
    )
...
def find_sep_clust_files(name, dir, method="umap"):
    ...
    for file in sorted(files):
        print(file, name)
        if (
            "_{}_".format(name) in file
            and "trimmed" in file
            and method in file
            and "sep_clust" in file
            and "png" in file
        ):
...
def get_text_metrics(
    text, size=DEFAULT_FONT_SIZE, family=DEFAULT_FONT_FAMILY, weight=DEFAULT_FONT_WEIGHT
):
    ...
    bbox = draw.textbbox((0, 0), text, font=imf)  # returns (x0, y0, x1, y1)
    text_width = bbox[2] - bbox[0]
    text_height = bbox[3] - bbox[1]

    s = (text_width, text_height)
    ...
    # ImageFont.truetype(family, size).getsize(text) #''.join(['a'] * len(text))) #text.upper())
