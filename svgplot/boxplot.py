...
def _add_boxplot(
    ...
    if label is not None:
        svg.add_text_bb(label, x=x, y=y - 20, orientation="v")

    # draw lines
    if data_points.size == 1:
        svg.add_line(
            x1=x - whisker_width / 2,
            x2=x + whisker_width / 2,
            y1=y + yaxis.w - yaxis.scale(data_points[0]),
            color=line_color,
            stroke=stroke,
        )
        return
    ...
    svg.add_line(
        x1=x,
        y1=y + yaxis.w - yaxis.scale(q3_iqr),
        y2=y + yaxis.w - yaxis.scale(q1_iqr),
        color=line_color,
        stroke=stroke,
    )
    svg.add_line(
    ...
    svg.add_line(
        x1=x - whisker_width / 2,
        x2=x + whisker_width / 2,
        y1=y + yaxis.w - yaxis.scale(q1_iqr),
        color=line_color,
        stroke=stroke,
    )
    ...
    svg.add_rect(
        x=x - width / 2,
        y=y + yaxis.w - yaxis.scale(q3),
        w=width,
        h=yaxis.scale(q3) - yaxis.scale(q1),
        stroke=stroke,
        fill=fill,
        fill_opacity=opacity,
        color=line_color,
        rounding=min(10, width / 2) if rounded else 0,
    )
...
def add_boxplot(
    ...
    _box_kws = core.kws(
        {
            ...
            "opacity": 1,
            "rounded": True,
            "median_style": MedianStyle.CIRCLE,
        },
        box_kws,
    )
    ...
    if _x_kws["show_axis"]:
        svg.add_line(
            x1=pos[0] - plot_width / 2 - x_gap - padding,
            x2=x2 - plot_width / 2 - x_gap + padding,
            y1=y1 + yaxis.w,
        )

    if show_legend:
        swarm._add_legend(svg, hue_order, palette, pos=(x2 - plot_width + 40, 0))
